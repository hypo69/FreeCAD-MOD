## Часть 3: Управление контентом (изображения и тексты)

Теперь мы создадим единый интерфейс для управления всеми загруженными данными: 
изображениями и текстовыми файлами. Пользователь сможет просматривать, редактировать, 
далять и связывать контент.
...
│       ├── load_image.svg
│       ├── load_text.svg
│       └── manage_content.svg   ← новая иконка
├── InitGui.py
├── ai_assistant_workbench.py    ← обновлён
└── data/                        ← все файлы здесь
```

---

## 📄 Обновлённый `ai_assistant_workbench.py` (Часть 3)

Мы добавим новую команду **«Manage Content»** и диалог с двумя вкладками: **Images** и **Texts**.
...
## 🖼 Новая иконка: `manage_content.svg`

Создайте файл `Resources/icons/manage_content.svg`:
...
- Удаление (можно выбрать несколько)

### Вкладка **Texts**:
- Список всех `.md` и `.txt` файлов
- **Редактирование** — открывает текст в редакторе
- Удаление

> 🔸 Все операции безопасны: подтверждение удаления, обработка ошибок, логирование в консоль FreeCAD.

---

## 📁 Структура данных остаётся той же:
...
1. Загрузите изображение и текст (как в Части 1 и 2)
2. Нажмите **Manage Content**
3. Переключайтесь между вкладками
4. Редактируйте тексты, удаляйте ненужное

---

## 🔜 Что будет в Части 4?

- **Связывание**: привязка текста к изображению (например, описание к схеме)
- **Экспорт всего проекта** в ZIP
- **Интеграция с ИИ**: отправка текста + изображения в LLM (локальный или облачный)

---

Готовы к Части 4? Напишите **«Да»** — и мы добавим **умную связку контента** и подготовим аддон к работе с искусственным интеллектом!